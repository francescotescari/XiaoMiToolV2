...
public class DeviceAnswers {
  ...
  private synchronized YesNoMaybe isInTwrpRecoveryInternal() {
    ...
    AdbRunner runner =
        AdbCommons.runner("shell twrp set " + key_test + " " + random, device.getSerial(), 6);
  ...
  public UnlockStatus getUnlockStatus() {
    ...
    UnlockStatus status =
        (UnlockStatus)
            device.getDeviceProperties().getFastbootProperties().get(DeviceProperties.X_LOCKSTATUS);
    if (status == null) {
      status =
          (UnlockStatus)
              device.getDeviceProperties().getAdbProperties().get(DeviceProperties.X_LOCKSTATUS);
    }
  ...
  public MiuiRom.Specie getCurrentSpecie() {
    ...
    MiuiVersion version =
        MiuiVersion.fromObject(device.getDeviceProperties().get(DeviceProperties.FULL_VERSION));
  ...
  public void updateStatus(Device.Status status) {
    if (Device.Status.SIDELOAD.equals(status)) {
      ...
      if (YesNoMaybe.YES.equals(getAnswer(HAS_STOCK_MIUI))
          && YesNoMaybe.YES.equals(this.hasTwrpRecovery())) {
  ...
  private YesNoMaybe isTwrpDataEncrypted(boolean mount_data) {
    ...
    String output =
        AdbCommons.adb_shell(
            "(ls -l /data || echo no_line_first) && (ls -l /sdcard || echo no_line_again)",
            device.getSerial(),
            4);
  ...
  public YesNoMaybe isWipeDataNeeded() {
    if ("unencrypted"
        .equalsIgnoreCase(
            (String) device.getDeviceProperties().getAdbProperties().get("ro.crypto.state"))) {
      return YesNoMaybe.NO;
    }
