...
public class Device {
  ...
  private final WaitSemaphore deviceActiveSem = new WaitSemaphore(),
      canBeAccessed = new WaitSemaphore(1, "can_be_accessed");
  ...
  private boolean reboot(Status toStatus, boolean force, boolean wait)
  ...
  public boolean rebootNoWait(Status toStatus, boolean force)
  ...
  private boolean rebootInternal(Status toStatus, boolean wait)
      throws InterruptedException, AdbException {
    deviceActiveSem.setPermits(0);
  ...
  private boolean fastbootReboot(Status toStatus, boolean wait)
      throws AdbException, InterruptedException {
    if (Status.FASTBOOT.equals(toStatus)) {
      ...
      if (FastbootCommons.oemRebootRecovery(serial)) {
        return !wait || rebootNoWait(toStatus);
      }
    }
    if (!Status.DEVICE.equals(toStatus) && !wait) {
      throw new AdbException(
          "Cannot reboot from fastboot to " + toStatus.toString() + " without waiting");
    }
    ...
    if (!rebootWait(Status.DEVICE)) {
      return isConnected && toStatus.equals(this.status);
    }
    return reboot(toStatus, false);
  }
  ...
  public void requireAccessibile() throws AdbException, InterruptedException {
    ...
    if (!(Status.FASTBOOT.equals(this.status)
        || Status.SIDELOAD.equals(this.status)
        || Status.RECOVERY.equals(this.status)
        || Status.DEVICE.equals(this.status))) {
      throw new AdbException(
          "The device cannot be managed right now: not accessibile status: "
              + this.status.toString());
    }
  }

  public boolean waitStatus(@NotNull Status status, int timeout)
      throws InterruptedException, AdbException {
