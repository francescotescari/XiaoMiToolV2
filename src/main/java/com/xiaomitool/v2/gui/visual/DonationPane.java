            ...
            new EventHandler<MouseEvent>() {
              ...
              public void handle(MouseEvent event) {
                ...
                if (donaCoor == null) {
                  ...
                  distanceFromDona =
                      GuiUtils.distanceBetween(
                              event.getSceneX(), event.getSceneY(), donaCoor[0], donaCoor[1])
                          + 1;
                }
                if (dontCoor == null) {
                  ...
                  distanceFromDont =
                      GuiUtils.distanceBetween(
                              event.getSceneX(), event.getSceneY(), dontCoor[0], dontCoor[1])
                          + 1;
                }
                smilePane.setHappiness(
                    75
                        + ((Math.pow(Math.abs(distanceFromDont - 7), 1.2)
                                - Math.pow(Math.abs(distanceFromDona - 7), 1.2)))
                            * (3200 / Math.pow(distanceFromDona + distanceFromDont - 70, 1.9)));
              }
            });
