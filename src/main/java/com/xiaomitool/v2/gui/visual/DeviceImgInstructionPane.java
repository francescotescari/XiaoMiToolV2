...
public class DeviceImgInstructionPane extends SidePane {
  ...
  public DeviceImgInstructionPane(
      double wantedHeight,
      double maxHeight,
      String[] texts,
      Image[] images,
      HashMap<Integer, Animatable.AnimationPayload> animations) {
    this.texts = texts;
    this.images = images;
  ...
  private void build(double wantedHeight, double maxHeight) {
    if (texts == null || images == null || texts.length != images.length || texts.length == 0) {
      throw new IllegalArgumentException(
          "Please, pass not null, not empty arrays with same num of elements");
    }
    ...
    deviceView =
        new AnimatableDeviceView(DeviceView.DEVICE_18_9, wantedHeight) {
          @Override
    ...
    this.setRight(
        GuiUtils.center(
            DeviceView.crop(
                deviceView,
                wantedHeight,
                wantedHeight > maxHeight ? (wantedHeight - maxHeight) / 2 : 0)));
  }
  ...
  private void loadStep() {
    ...
    String toLoadText =
        LRes.STEP.toString() + " [" + (index + 1) + "/" + (texts.length) + "]:\n" + texts[index];
    if (index == 0) {
      prevButton.setDisable(true);
    } else {
