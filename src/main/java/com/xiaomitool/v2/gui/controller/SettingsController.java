...
public class SettingsController extends DefaultController {
  ...
  @FXML private Label LABEL_DOWNLOAD, LABEL_EXTRACT, TEXT_DOWNLOAD, TEXT_EXTRACT;
  @FXML private Button BUTTON_DOWNLOAD, BUTTON_EXTRACT, BUTTON_CLEAR, BUTTON_RESET, BUTTON_FEEDBACK;
  @FXML private CheckBox CHECK_SAVE_LOGIN;
  @FXML private StackPane WHOLE;
  @FXML private ComboBox<String> REGION_COMBO, LANG_COMBO;
    ...
    if (feedbackPopup == null) {
      ...
      textArea.setTextFormatter(
          new TextFormatter<String>(
              change -> change.getControlNewText().length() <= 500 ? change : null));
            ...
            public void handle(ActionEvent event) {
              ...
              if ((text == null || text.isEmpty()) && !sendLogFile) {
                ...
                textArea.setPromptText(
                    "You have to either send a text log or include the log file or both");
              } else {
                button.setDisable(true);
                        ...
                        new Runnable() {
                          ...
                          public void run() {
                            ...
                            try {
                              ...
                              Platform.runLater(
                                  () -> settingsToast.toast(LRes.FEEDBACK_SENT.toString()));
                            } catch (Exception e) {
                              Log.error("Failed to send the feedback: " + e.getMessage());
                              Log.exc(e);
                              if (feedbackToast != null) {
                                Platform.runLater(
                                    () -> feedbackToast.toast(LRes.FEEDBACK_ERROR.toString()));
                              }
                            }
