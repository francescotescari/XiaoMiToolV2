...
public class DeviceTableEntry {
  ...
  public DeviceTableEntry(
      String serial, String codename, String status, String brand, String model) {
  ...
  public DeviceTableEntry(Device device) {
    this(
        device.getSerial(),
        device.isConnected() ? device.getStatus() : null,
        device.getDeviceProperties());
  }

  public DeviceTableEntry(
      String serial, Device.Status deviceStatus, DeviceProperties deviceProperties) {
    this(
        serial,
        deviceProperties.getCodename(true),
        deviceStatus == null ? "disconnected" : deviceStatus.toString(),
        (String) deviceProperties.get(DeviceProperties.BRAND),
        (String) deviceProperties.get(DeviceProperties.MODEL));
  }
