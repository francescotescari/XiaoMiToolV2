...
public class AdbSideloadTask extends Task {
  ...
  protected void startInternal() {
    ...
    if (fileToSideload == null) {
      this.error(
          new InstallException("null file to sideload", InstallException.Code.INTERNAL_ERROR));
    }
    ...
    if (runner.getExitValue() != 0) {
      error(
          new InstallException(
              new AdbException("adb sideload exited with status: " + runner.getExitValue())));
      return;
    }
    ...
    if (out.contains("complete")) {
      ...
      error(
          new AdbException(
              "output of sideload is abort, progress: "
                  + getLatestUpdate()
                  + "/"
                  + getTotalSize()));
      return;
    ...
    if (getLatestUpdate() >= 0 && getTotalSize() > 0) {
      ...
      if (percent < 90) {
        error(
            new AdbException(
                "sideload completed only for "
                    + percent
                    + "%, the installation might not completed succesfully"));
        return;
      }
    }
    finished(fileToSideload);
