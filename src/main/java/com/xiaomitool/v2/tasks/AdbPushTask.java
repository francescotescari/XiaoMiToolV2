...
public class AdbPushTask extends Task {
  ...
  public AdbPushTask(
      UpdateListener listener, String sourceFile, String destinationPath, String device) {
    this(listener, new File(sourceFile), destinationPath, device);
  }

  public AdbPushTask(
      UpdateListener listener, File sourceFile, String destinationPath, String device) {
    super(listener);
  ...
  protected void startInternal() {
    ...
    if (runner.getExitValue() != 0) {
      String mot =
          lastLine.pointed != null
              ? lastLine.pointed.toString()
              : ("exit code " + runner.getExitValue());
