...
public class ExtractionTask extends Task {
  ...
  private File extractGzip() throws IOException {
    InputStream stream =
        new GZIPInputStream(
            new FeedbackInputStream(
                new BufferedInputStream(new FileInputStream(fileToExtract), FILE_READ_BLOCK_SIZE),
                new RunnableWithArg() {
                  @Override
            ...
            new GZIPInputStream(
                new FeedbackInputStream(
                    new BufferedInputStream(
                        new FileInputStream(fileToExtract), FILE_READ_BLOCK_SIZE),
                    arg -> onUpdateInternal((int) arg))));
    return extractTarInternal(tarIn, keepPath);
  }

  private File extractTarInternal(TarArchiveInputStream tarIn, boolean keepPath)
      throws IOException {
    ArchiveEntry entry;
    while ((entry = tarIn.getNextEntry()) != null) {
      File tmpFile =
          new File(
              dirExtractTo, keepPath ? entry.getName() : FilenameUtils.getName(entry.getName()));
  ...
  public String toString() {
    ...
    try {
      builder
          .append("Extraction entry")
          .append(n)
          .append("Input file: ")
          .append(fileToExtract.toString())
          .append(n)
          .append("Output dir: ")
          .append(dirExtractTo.toString())
          .append(n)
          .append("Extraction type: ")
          .append(type.toString())
          .append(n)
          .append("File size: ")
          .append(fileToExtract.length());
    } catch (Throwable t) {
