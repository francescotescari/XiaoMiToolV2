...
public abstract class Installable extends CommandClass implements Choiceable, InstallObject {
  ...
  public Installable(
      Type type, boolean isOfficial, String unique, boolean needDownload, boolean needExtraction) {
    this.type = type;
  ...
  protected Object downloadInternal(UpdateListener listener) throws Exception {
    ...
    Task task =
        new DownloadTask(
            listener,
            getDownloadUrl(),
            downloadFilename == null ? null : SettingsUtils.getDownloadFile(downloadFilename));
    TaskManager.getInstance().startSameThread(task);
    if (!task.isFinished()) {
      throw new RomException(
          "Failed to download file: "
              + ((task.getError() != null) ? task.getError().getMessage() : "null error"));
    }
  ...
  public String toLogString() {
    return this.getClass().getSimpleName()
        + " - "
        + StrUtils.str(this.getType())
        + "[url:"
        + StrUtils.str(this.downloadUrl)
        + ",dl:"
        + StrUtils.str(downloadedFile)
        + ",final:"
        + StrUtils.str(finalFile)
        + "]";
  }
