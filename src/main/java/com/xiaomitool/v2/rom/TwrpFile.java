...
public class TwrpFile extends Installable {
  ...
  protected File extractInternal(UpdateListener listener) throws Exception {
    ...
    ExtractionTask task =
        new ExtractionTask(listener, file, null, ExtractionTask.ExtractionType.ZIP, false);
    TaskManager manager = TaskManager.getInstance();
    manager.startSameThread(task);
    if (!task.isFinished()) {
      throw new RomException(
          "Extraction task failed: "
              + (task.getError() == null ? "null error" : task.getError().getMessage()));
    }
    ...
    if (finalFile == null) {
      Exception e =
          new IOException("Could not find a valid twrp image file in extracted directory");
      listener.onError(e);
      throw e;
    }
    return finalFile;
  ...
  public String getTitle() {
    return LRes.TWRP_RECOVERY.toString()
        + (this.getDownloadUrl() != null
            ? (" - " + FilenameUtils.getName(this.getDownloadUrl()))
            : "");
  }
  ...
  public String getText() {
    return this.getDownloadUrl() != null
        ? LRes.TWRP_AUTO_DOWNLOAD.toString()
        : LRes.TWRP_SELECT_MANUAL.toString();
  }
