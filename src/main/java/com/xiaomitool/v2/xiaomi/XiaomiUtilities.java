...
public class XiaomiUtilities {
  private static final Pattern P_IS_REDMI =
      Pattern.compile("^((xiao)?mi)?\\s*redmi(\\b)", Pattern.CASE_INSENSITIVE);
  private static final Pattern P_IS_POCO =
      Pattern.compile("^((xiao)?mi)?\\s*poco(\\b)", Pattern.CASE_INSENSITIVE);
  private static final Pattern P_IS_MI =
      Pattern.compile("^((xiao)?mi)?\\s*mi(\\b)", Pattern.CASE_INSENSITIVE);
  private static final Pattern P_IS_NOTE =
      Pattern.compile("\\s+note\\s+", Pattern.CASE_INSENSITIVE);
  ...
  public static class DeviceName {
    ...
    private DeviceName(
        boolean is_mi, boolean is_redmi, boolean is_poco, boolean is_note, String rest) {
      this.is_mi = is_mi;
    ...
    public static List<DeviceName> parse(String name) {
      ...
      for (String part : parts) {
        ...
        tmp = tmp.trim();
        if (("pro".equalsIgnoreCase(tmp) || "prime".equalsIgnoreCase(tmp))
            && lastUnparsed != null) {
    ...
    public boolean equals(Object obj) {
      ...
      boolean neq =
          (this.is_note ^ dev.is_note)
              || (this.is_redmi ^ dev.is_redmi)
              || (this.is_poco ^ dev.is_poco)
              || (this.is_mi ^ dev.is_mi);
      return !neq && this.not_parsed.equalsIgnoreCase(dev.not_parsed);
    }

    @Override
    public int hashCode() {
      return this.not_parsed.hashCode()
          + (is_mi ? 1 : 0)
          + (is_redmi ? 2 : 0)
          + (is_poco ? 4 : 0)
          + (is_note ? 8 : 0);
