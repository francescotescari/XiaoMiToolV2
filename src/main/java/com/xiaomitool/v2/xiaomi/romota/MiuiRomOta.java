...
public class MiuiRomOta {
  ...
  public static @NotNull HashMap<Kind, MiuiZipRom> otaV3_request(RequestParams params)
      throws XiaomiProcedureException, CustomHttpException {
    return otaV3_request(params, false);
  }

  public static @NotNull HashMap<Kind, MiuiZipRom> otaV3_request(
      RequestParams params, boolean throwOnCode)
      throws XiaomiProcedureException, CustomHttpException {
    ...
    try {
      String[] tokens =
          XiaomiKeystore.getInstance()
              .requireServiceKeyAndToken(params.isInternational() ? "miuiota_intl" : "miuiromota");
      mode = 2;
      key = tokens[0];
    ...
    String url =
        international
            ? "https://update.intl.miui.com/updates/miotaV3.php"
            : "https://update.miui.com/updates/miotaV3.php";
    ...
    if (userId != null && !userId.isEmpty()) {
      try {
        ...
        throw new XiaomiProcedureException(
            String.format(
                "[otaV3_request] Encryption of user id failed: %s key=%s",
                e.getMessage(), XiaomiKeystore.KEY_MIOTAV3));
      }
    }
    String postData;
    String json = null;
    ...
    try {
      ...
      throw new XiaomiProcedureException(
          String.format("[otaV3_request] Encryption failed: %s key=%s", e.getMessage(), key));
    }
    ...
    try {
      ...
      throw new XiaomiProcedureException(
          "[otaV3_request] Response is invalid: decryption failed: " + e.getMessage());
    }
    ...
    Kind[] entries =
        new Kind[] {Kind.LATEST, Kind.CURRENT, Kind.INCREMENTAL, Kind.PACKAGE, Kind.CROSS};
    ...
    if (throwOnCode) {
      try {
        ...
        if (code != 2000) {
          throw new XiaomiProcedureException(
              "Ota response code is " + code + ", description: " + message,
              XiaomiProcedureException.ExceptionCode.NOT_ALLOWED,
              message);
        }
      } catch (Throwable t) {
        ...
        if (map.isEmpty()
            || (params.getPkg() != null
                && !params.getPkg().isEmpty()
                && !map.containsKey(Kind.PACKAGE))) {
          throw new XiaomiProcedureException(
              "Failed to get response code: " + t.getMessage(),
              XiaomiProcedureException.ExceptionCode.NOT_ALLOWED,
              "Cannot get ota response code");
        }
      }
    }
    return map;
  ...
  private static MiuiZipRom parseZipRomFromJson(JSONObject object, Kind kind, MiuiRom.Specie specie)
      throws XiaomiProcedureException {
    try {
      ...
      return new MiuiZipRom(
          filename,
          new MiuiVersion(s_version),
          bbranch,
          new Codebase(s_codebase),
          md5,
          token,
          kind,
          descriptionUrl,
          specie.toBranch(bbranch));
    } catch (Exception e) {
      return new MiuiZipRom(true);
    }
  }

  private static MiuiTgzRom parseTgzRomFromJson(JSONObject object, Kind kind, MiuiRom.Specie specie)
      throws XiaomiProcedureException {
    try {
      String filename = object.getString(OTA_FILENAME);
      String s_version = object.getString(OTA_VERSION);
  ...
  public static MiuiTgzRom latestFastboot_request(RequestParams params)
      throws XiaomiProcedureException, CustomHttpException {
    ...
    String url =
        String.format(
            "http://update.miui.com/updates/miota-fullrom.php?d=%s&b=%s&r=%s&n=%s&l=%s",
            device, branch.getCode(), region, n, lang);
    String result;
    ...
    try {
      ...
      throw new XiaomiProcedureException(
          "[latestFastboot_request] Failed to parse json: " + e.getMessage());
    }
    Mirrors mirrors = new Mirrors();
    try {
  ...
  public static MiuiZipRom latestRecovery_request(RequestParams params)
      throws XiaomiProcedureException, CustomHttpException {
    ...
    EasyResponse response;
    response =
        new MiuiSaltedRequest(salt)
            .url(url)
            .field("sid", "miassistant")
            .field("d", device)
            .field("c", codebase)
            .field("f", branch)
            .exec();
    ...
    try {
      ...
      throw new XiaomiProcedureException(
          "[latestRecovery_request] Json parsing failed: " + e.getMessage());
    }
    int code = obj.optInt("code", 0);
    if (code == 0) {
      throw new XiaomiProcedureException(
          "[latestRecovery_request] Invalid return code from json: zero or not present");
    }
    ...
    try {
      ...
      throw new XiaomiProcedureException(
          "[latestRecovery_request] Returned download url is malformed or not recognized");
    }
  }

  public static MiuiTgzRom latestFastboot2_request(RequestParams params)
      throws XiaomiProcedureException, CustomHttpException {
    String device = params.getModDevice();
    Branch branch = params.getBranch();
    String region = params.getFastbootRegion();
    ...
    String
        url =
            String.format(
                "https://update.miui.com/updates/v1/fullromdownload.php?d=%s&b=%s&r=%s&n=%s",
                device, branch.getCode(), region, n),
        dl;
    ...
    if (!XiaomiUtilities.isFastbootFile(dl)) {
      throw new XiaomiProcedureException(
          "[latestFastboot2_request] Redirect url is not a fastboot file");
    }
    try {
      return new MiuiTgzRom(dl, params.getSpecie());
    } catch (RomException e) {
      throw new XiaomiProcedureException(
          "[latestFastboot2_request] Returned download url is malformed or not recognized");
    }
  }

  public static JSONObject deviceNames_request()
      throws XiaomiProcedureException, CustomHttpException {
    String salt = "ZjYyYWIxYzNhM2I=";
    String url = "https://update.miui.com/updates/v1/devinfo.php";
    EasyResponse response;
    response =
        new MiuiSaltedRequest(salt).url(url).field("sid", "miassistant").field("t", "1001").exec();
    String body = response.getBody();
    JSONObject obj;
    try {
      obj = new JSONObject(body);
      ...
      throw new XiaomiProcedureException(
          "[deviceNames_request] Json parsing failed: " + e.getMessage());
    }
  }
}
