...
public class MiuiCommunityApis {
  ...
  public static Map<String, Integer> getDevicesCommunityIds()
      throws CustomHttpException, XiaomiProcedureException {
    EasyResponse response = EasyHttp.get(IDS_URL);
    HashMap<String, Integer> result = new HashMap<>();
    if (!response.isAllRight()) {
      throw new XiaomiProcedureException(
          "Invalid response code or body",
          XiaomiProcedureException.ExceptionCode.EXCEPTION,
          response.getBody());
    }
    try {
      ...
      JSONArray devices =
          jsonObject.getJSONObject("data").getJSONObject("phone_data").getJSONArray("phone_list");
      ...
      throw new XiaomiProcedureException(
          "Failed to parse community api response: " + t.getMessage(),
          XiaomiProcedureException.ExceptionCode.EXCEPTION,
          t.toString());
    }
    return result;
