...
public class UnlockRequest {
  ...
  public String exec() throws XiaomiProcedureException, CustomHttpException {
    ...
    signHmac =
        XiaomiCrypto.cloudService_signHmac(
            XiaomiCrypto.UNLOCK_HMAC_KEY, "POST", path, params.sorted().toString());
    params.put("sign", signHmac);
    String key = keyToken[0];
    String serviceToken = keyToken[1];
    Log.info("Unlock request (" + this.path + ") params: " + params.sorted());
    ...
    if (this.encrypt) {
      try {
        ...
        throw new XiaomiProcedureException(
            "[UnlockRequest.exec] Cannot encrypt post params: " + e.getMessage());
      }
    ...
    EasyResponse response =
        new EasyHttp()
            ...
            .fields(params)
            .headers(headers)
            .userAgent("XiaomiPCSuite")
            .cookies(XiaomiKeystore.getInstance().requireServiceCookies(SERVICE_NAME))
            .exec();
    if (!response.isAllRight()) {
      throw new XiaomiProcedureException(
          "[UnlockRequest.exec] Invalid server respose: code: "
              + response.getCode()
              + ", lenght: "
    ...
    if (encrypt) {
      try {
        ...
        throw new XiaomiProcedureException(
            "[UnlockRequest.exec] Cannot decrypt response data: " + e.getMessage());
      }
      try {
  ...
  public void addNonce() throws XiaomiProcedureException, CustomHttpException {
    ...
    try {
      ...
      if (code != 0) {
        throw new XiaomiProcedureException(
            "[UnlockRequest.addNonce] Response code of nonce request is not zero: " + code);
      }
      String nonce = obj.getString("nonce");
      params.put("nonce", nonce);
    } catch (Exception e) {
      throw new XiaomiProcedureException(
          "[UnlockRequest.addNonce] Exception while parsing nonce response: " + e.getMessage());
    }
  }
