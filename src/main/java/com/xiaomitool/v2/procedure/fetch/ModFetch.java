...
public class ModFetch {
  private static final String MAGISK_UPDATE_URL =
        ...
        try {
          ...
          if (!response.isAllRight()) {
            throw new InstallException(
                "Failed to get latest stable magisk data from static url",
                InstallException.Code.INFO_RETRIVE_FAILED,
                new Exception("Returned status code: " + response.getCode()));
          }
          ...
          ApkFileInstallable app =
              new ApkFileInstallable("MagiskManager " + aVersion, aUrl, Device.Status.RECOVERY) {
                @Override
          ...
          throw new InstallException(
              "Failed to parse latest stable magisk data",
              InstallException.Code.INFO_RETRIVE_FAILED,
              e);
        }
      }
