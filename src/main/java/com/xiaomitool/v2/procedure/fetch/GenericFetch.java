...
public class GenericFetch {
  ...
  public static RInstall fetchAllOfficial(Device device) {
    ...
    LinkedHashSet<MiuiRom.Specie> speciesToSearch =
        MiuiRom.Specie.listToSearchSpecies(region, codename);
    return RNode.sequence(
        FastbootFetch.findAllLatestFastboot(speciesToSearch),
        StockRecoveryFetch.allLatestOta(speciesToSearch));
  }
      ...
      public void run(ProcedureRunner runner)
        ...
        if (!file.exists()) {
          throw new InstallException(
              "File not found: " + file.toString(), InstallException.Code.FILE_NOT_FOUND);
        }
        String md5 = "";
        ...
        try (BufferedInputStream in = new BufferedInputStream(new FileInputStream(file))) {
          ...
          throw new InstallException(
              "Failed to calculate file md5: " + file, InstallException.Code.HASH_FAILED, e);
        }
        if (StrUtils.isNullOrEmpty(md5)) {
          throw new InstallException(
              "Failed to calculate file md5: " + file.toString(),
              InstallException.Code.HASH_FAILED,
              "Null or empty result hash");
        }
  ...
  public static RInstall fetchAllUnofficial() {
    return RNode.skipOnException(
        RNode.sequence(
            GenericFetch.fetchXiaomieuRom(Branch.STABLE),
            new RInstall() {
              @Override
              public void run(ProcedureRunner runner)
                  throws InstallException, RMessage, InterruptedException {
        ...
        RNode.sequence(
            GenericFetch.fetchXiaomieuRom(Branch.DEVELOPER),
            new RInstall() {
              @Override
              public void run(ProcedureRunner runner)
                  throws InstallException, RMessage, InterruptedException {
  ...
  public static RInstall fetchDeviceCodename(String keyDest) {
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (res.isEmpty()) {
          throw new InstallException(
              "Empty codename list from api", InstallException.Code.INFO_RETRIVE_FAILED);
        }
