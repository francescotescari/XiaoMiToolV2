...
public class RecoverInstall {
  public static RInstall recoverFastboot() {
    return RNode.sequence(
        ...
        ConfirmationProcedure.suggestUnlockBootloader(
            LRes.SUGGEST_UNLOCK_BL_RECOVER.toString(LRes.CONTINUE.toString())),
        RNode.conditional(
      ...
      public void run(ProcedureRunner runner)
        ...
        if (Device.Status.FASTBOOT.equals(status)) {
          ...
          throw new InstallException(
              "The device is not in fastboot or recovery mode ("
                  + status
                  + "), you should not use this feature",
              InstallException.Code.CANNOT_INSTALL);
        }
        Procedures.pushRInstallOnStack(
            runner,
            ChooseProcedure.alternativeBackupMethod(
                nextProcedure, altProcedure, LRes.ERROR_WHILE_RECOVERING.toString()));
      }
    }.next();
  }
}
