...
public class GenericInstall {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (StrUtils.isNullOrEmpty(installable.getDownloadUrl())) {
          throw new InstallException(
              "Download failed: empty or null download url",
              InstallException.Code.DOWNLOAD_FAILED,
              "Installable "
                  + installable
                  + " has no download url but it is market as it should have");
        }
        ...
        defProgressPane.setContentText(
            LRes.DOWNLOADING_ROM_FILE.toString()
                + "\n"
        ...
        try {
          ...
          throw new InstallException(
              "Download task failed: " + e.getMessage(), InstallException.Code.DOWNLOAD_FAILED, e);
        } finally {
    ...
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (installable.getDownloadedFile() == null) {
          throw new InstallException(
              "Extract failed: null downloaded file",
              InstallException.Code.EXTRACTION_FAILED,
              "Installable "
                  + installable
                  + " has no extract file but it is market as it should have");
        }
        Log.info("Extracting file: " + installable.getDownloadedFile());
        ProgressPane.DefProgressPane defProgressPane = new ProgressPane.DefProgressPane();
        defProgressPane.setContentText(
            LRes.EXTRACTING_ROM_FILE + "\n" + installable.getDownloadedFile().toString());
        ...
        try {
          ...
          throw new InstallException(
              "Extraction task failed: " + e.getMessage(),
              InstallException.Code.EXTRACTION_FAILED,
              e);
        } finally {
          WindowManager.removeTopContent();
        ...
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            if (installable == null && isProcedure != null && isProcedure == true) {
              Log.warn(
                  "There is no installable, a procedure was selected, this probably means that a procedure has already finished, skip the installation part");
              return;
            }
            installable = Procedures.requireInstallable(runner);
            Procedures.pushRInstallOnStack(runner, installable.getInstallProcedure());
            Log.info(
                "Installation procedure to run: " + installable.getInstallProcedure().toString(1));
          }
        },
  ...
  public static RInstall satisfyAllRequirements() {
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (stashed == null) {
          Log.info(
              "Stashing the installable to satisfy the requirements: " + installable.toLogString());
          runner.setContext(KEY_STASHED_INSTALLABLE, installable);
        }
        StatedProcedure toSatisfy =
            InstallationRequirement.satisfyNextRequirement(
                Procedures.requireDevice(runner), installable);
        ...
        while (toSatisfy != null) {
          if (toSatisfy.getInstallProcedure() != null) {
            if (YesNoMaybe.YES.equals(device.getAnswers().isNeedDeviceDebug())
                && Procedures.stillNeedUsbDebug(runner, toSatisfy)) {
              Log.warn(
                  "Satisfying the requirement resetted the phone, you need to enable usb debug again");
          ...
          if (Objects.equals(copy, toSatisfy)) {
            throw new InstallException(
                "Trying to satisfy a requirement that should had been just satisfied: " + copy,
                InstallException.Code.INTERNAL_ERROR,
                "Past requirement: " + copy + ", current requirement: " + toSatisfy);
          }
          try {
            Log.info("The device might be rebooting right now, lets wait it for 30 seconds");
            ManageDevice.waitDevice(30, Device.Status.DEVICE)
        ...
        if (stashed != null) {
          if (YesNoMaybe.YES.equals(device.getAnswers().isNeedDeviceDebug())
              && Procedures.stillNeedUsbDebug(runner, stashed)) {
            Log.warn(
                "Satisfying the requirement resetted the phone, you need to enable usb debug again before installing stashed installable");
            RebootDevice.rebootNoWaitIfConnected().run(runner);
            ActionsDynamic.WAIT_USB_DEBUG_ENABLE(device).run();
          }
  ...
  public static RInstall updateDeviceStatus(
      Boolean isUnlocked, Boolean hasTwrp, Boolean hasUsbDebug) {
    return updateDeviceStatus(isUnlocked, hasTwrp, hasUsbDebug, null);
  }

  public static RInstall updateDeviceStatus(
      Boolean isUnlocked, Boolean hasTwrp, Boolean hasUsbDebug, Boolean hasStockMiui) {
    return new RInstall() {
      @Override
      public void run(ProcedureRunner runner)
          throws InstallException, RMessage, InterruptedException {
        Log.info(
            "Updating the device status: isUnlocked: "
                + isUnlocked
                + ", hasTwrp: "
                + hasTwrp
                + ", hasUsbDebug: "
        ...
        if (isUnlocked != null) {
          device
              ...
              .put(
                  DeviceProperties.X_LOCKSTATUS,
                  isUnlocked ? UnlockStatus.UNLOCKED : UnlockStatus.LOCKED);
        }
        if (hasTwrp != null) {
          device
              .getAnswers()
              .setAnswer(DeviceAnswers.HAS_TWRP, hasTwrp ? YesNoMaybe.YES : YesNoMaybe.NO);
        }
        if (hasUsbDebug != null) {
        ...
        if (hasStockMiui != null) {
          device
              .getAnswers()
              .setAnswer(
                  DeviceAnswers.HAS_STOCK_MIUI, hasStockMiui ? YesNoMaybe.YES : YesNoMaybe.NO);
        }
      }
    };
  }
  ...
  public static RInstall restartMain(RInstall startFromHere) {
    ...
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (start == null) {
          throw new InstallException(
              "Cannot restart procedure from null", InstallException.Code.INTERNAL_ERROR);
        }
                  ...
                  public void run() {
                    try {
                      ...
                      ActionsDynamic.START_PROCEDURE(
                              device, RNode.sequence(unstashContext(), start), runner, start)
                          .run();
  ...
  public static RInstall showUserAndRestart(
      String message, boolean throwUplevel, RInstall startFromHere) {
    return new RInstall() {
      @Override
      public void run(ProcedureRunner runner)
          throws InstallException, RMessage, InterruptedException {
        ButtonPane buttonPane =
            new ButtonPane(
                throwUplevel
                    ? new LRes[] {LRes.OK_UNDERSTAND, LRes.TRY_AGAIN}
                    : new LRes[] {LRes.OK_UNDERSTAND});
  ...
  public static RInstall recoverMain() {
    return RNode.sequence(
        ManageDevice.requireAdbCheckService(),
        ConfirmationProcedure.confirmPhoneCharged(true),
        ManageDevice.recoverSelectDevice(),
        recoverDeviceStart());
  }

  public static RInstall recoverDeviceStart() {
    return RNode.sequence(
        OtherProcedures.restoreInstallPane(),
        OtherProcedures.text(LRes.STARTING_RECOVERY_PROC.toString()),
        stashContext(),
        ManageDevice.requireDeviceCodename(),
        RecoverInstall.recoverDeviceOrderSubProcedures(),
