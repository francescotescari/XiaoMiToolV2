...
public class TwrpInstall {
    ...
    return RNode.sequence(
        ...
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            String flashOutput =
                FastbootCommons.flash(device.getSerial(), installable.getFinalFile(), "recovery");
            ...
            if (flashOutput == null || flashOutput.startsWith("err:")) {
              throw new InstallException(
                  new AdbException(
                      "Fastboot flash recovery failed, output:" + StrUtils.str(flashOutput)));
            }
  ...
  public static RInstall checkIfIsInTwrp() {
    return RNode.sequence(
        OtherProcedures.sleep(4000),
        RebootDevice.requireRecovery(),
        OtherProcedures.sleep(2000),
        ManageDevice.requireAccessible(true),
        new RInstall() {
          @Override
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
            ...
            if (YesNoMaybe.NO.equals(device.getAnswers().isInTwrpRecovery())) {
              throw new InstallException(
                  "Failed to get twrp commands running, are you in twrp? Wait for twrp boot and try again",
                  InstallException.Code.TWRP_INSTALL_FAILED,
                  "Device is not in twrp or openrecoveryscripts not working atm. Device Status: "
                      + device.getStatus()
                      + " - connected: "
                      + device.isConnected());
            }
    ...
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (!AdbCommons.formatBootdevicePartition("userdata", device.getSerial(), 10)) {
          throw new InstallException(
              new AdbException(
                  "Failed to format userdata: " + AdbCommons.getLastError(device.getSerial())));
        }
  ...
  public static RInstall installZip(boolean formatDataIfNecessary) {
    return RNode.sequence(
        RebootDevice.requireRecovery(),
        formatDataIfNecessary ? formatDataIfEncryptedAndNeeded() : Procedures.doNothing(),
        new RInstall() {
          @Override
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
        ...
        AdbInstall.pushInstallableFile(),
        new RInstall() {
          @Override
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
            ...
            if (eraseData) {
              Log.warn(
                  "The file to install is a large zip file, likely a zip rom, we must erase data after that");
            } else {
              Log.info(
                  "The file to install is a small zip file, likely a mod zip, we don't have to erase data after that");
            }
            ...
            if (task.getError() != null || !task.isFinished()) {
              throw new InstallException(
                  "Failed to install the zip file on the device: task failed: "
                      + ((task.getError() != null) ? task.getError().getMessage() : "not finished"),
                  InstallException.Code.TWRP_INSTALL_FAILED,
                  task.getError());
            }
          }
    ...
    return RNode.sequence(
        ...
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            if (!result) {
              throw new InstallException(
                  "Failed to wipe cache: twrp command failed",
                  InstallException.Code.WIPE_FAILED,
    ...
    return RNode.sequence(
        ...
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            if (!result) {
              throw new InstallException(
                  "Failed to wipe cache and data: twrp command failed",
                  InstallException.Code.WIPE_FAILED,
                  AdbCommons.getLastError(device.getSerial()));
            }
          }
  ...
  public static RInstall installApkViaTwrp() {
    return RNode.sequence(
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
            ApkFileInstallable installable =
                (ApkFileInstallable) Procedures.requireInstallable(runner).orig();
        ...
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            Device device = Procedures.requireDevice(runner);
            String res =
                AdbCommons.adb_shellWithOr(
                    "cp \"" + apkPath + "\" /data/app/" + packageName + "-1",
                    device.getSerial(),
                    5);
            if (res == null) {
