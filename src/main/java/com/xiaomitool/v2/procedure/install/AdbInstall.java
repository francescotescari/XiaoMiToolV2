      ...
      public void run(ProcedureRunner runner)
        ...
        String outputPath =
            destinationpath.endsWith("/")
                ? (destinationpath + FilenameUtils.getName(fileToPush.toString()))
                : destinationpath;
        ...
        AdbPushTask pushTask =
            new AdbPushTask(listener, fileToPush, outputPath, device.getSerial());
        ...
        if (pushTask.getError() != null) {
          throw new InstallException(
              new AdbException(
                  "Failed to push file on device: " + pushTask.getError().getMessage()));
        }
    ...
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (!AdbCommons.mkdir(destinationpath, device.getSerial())) {
          throw new InstallException(
              "Failed to create remote directory: " + destinationpath,
