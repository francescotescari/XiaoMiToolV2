...
public class ManageDevice {
  ...
  public static RInstall checkIfTwrpInstalled() {
    return RNode.sequence(
        requireAccessible(),
        OtherProcedures.sleep(1000),
        RNode.fallback(
            RNode.sequence(
                RebootDevice.rebootRecovery(true, false, 30),
  ...
  public static RInstall waitRequireAccessible(
      int timeout, Device.Status showTextExpectedDeviceStatus) {
    return RNode.sequence(
        RNode.setSkipOnException(waitDevice(timeout, showTextExpectedDeviceStatus)),
      ...
      public void run(ProcedureRunner runner)
        ...
        if (showTextExpectedDeviceStatus != null) {
          String connectionType =
              Device.Status.DEVICE.equals(showTextExpectedDeviceStatus)
                  ? "ADB"
                  : showTextExpectedDeviceStatus.toString();
        ...
        if (device.waitActive(t) == null) {
          throw new InstallException(
              "Waited device for " + timeout + " seconds but it seems still disconnected",
              InstallException.Code.WAIT_DEVICE_TIMEOUT);
        }
      }
    };
  }
  ...
  public static RInstall selectDeviceCodename() {
    return RNode.sequence(
        GenericFetch.fetchDeviceCodename(DEIVCE_CODENAME_KEY),
        new RInstall() {
          @Override
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
            Device device = Procedures.requireDevice(runner);
            HashMap<String, String> devicesCodename =
                (HashMap<String, String>) runner.getContext(DEIVCE_CODENAME_KEY);
            if (devicesCodename == null) {
              GenericFetch.fetchDeviceCodename(DEIVCE_CODENAME_KEY).run(runner);
              devicesCodename =
                  (HashMap<String, String>) runner.requireContext(DEIVCE_CODENAME_KEY);
            }
  ...
  public static RInstall recoverSelectDevice() {
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
          throws InstallException, RMessage, InterruptedException {
        int res =
            ActionsDynamic.SEARCH_SELECT_DEVICES(
                    Device.Status.FASTBOOT, Device.Status.RECOVERY, Device.Status.SIDELOAD)
                .run();
    ...
    return new RInstall() {
      ...
      public void run(ProcedureRunner runner)
        ...
        if (!UnlockStatus.UNLOCKED.equals(device.getAnswers().getUnlockStatus())) {
          throw new InstallException(
              "Bootloader locked. It is necessary to unlock the bootlaoder to continue.",
