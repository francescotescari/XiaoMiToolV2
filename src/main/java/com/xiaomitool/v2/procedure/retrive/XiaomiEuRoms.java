...
public class XiaomiEuRoms {
  public static final String[] SOURCEFORGE_MIRRORS = new String[] {"master"};
  ...
  public static ZipRom latest(RequestParams params) throws CustomHttpException, RomException {
    ...
    if (!device.matches("aries|cancro|virgo|hermes|mocha|latte|hennessy")) {
      if (Branch.STABLE.equals(branch)) {
        format =
            "https://%1$s.dl.sourceforge.net/project/xiaomi-eu-multilang-miui-roms/xiaomi.eu/MIUI-STABLE-RELEASES/MIUI%2$s/xiaomi.eu_multi_%3$s_%4$s_v%5$d-%6$s.zip";
      } else {
        format =
            "https://%1$s.dl.sourceforge.net/project/xiaomi-eu-multilang-miui-roms/xiaomi.eu/MIUI-WEEKLY-RELEASES/%2$s/xiaomi.eu_multi_%3$s_%4$s_v%5$d-%6$s.zip";
      }
    } else {
      if (Branch.STABLE.equals(branch)) {
        format =
            "https://basketbuild.com/uploads/devs/ZduneX25/roms/MIUI%2$s/xiaomi.eu_multi_%3$s_%4$s_v%5$d-%6$s.zip";
      } else {
        format =
            "https://basketbuild.com/uploads/devs/ZduneX25/roms/%2$s/xiaomi.eu_multi_%3$s_%4$s_v%5$d-%6$s.zip";
      }
    }
    String downloadUrl =
        String.format(format, mirror, shortVersion, name, version, bigVersion, codebase);
    return new ZipRom(downloadUrl, miuiVersion, new Codebase(codebase)) {
      @Override
      public String getTitle() {
        return branch == null
            ? LRes.XIAOMIEU_TITLE.toString()
            : (LRes.XIAOMIEU_TITLE.toString() + " - " + LRes.branchToString(branch));
      }

      @Override
      public String getText() {
        return this.getMiuiVersion() == null
            ? LRes.XIAOMIEU_AUTO_DOWNLOAD.toString()
            : (LRes.XIAOMIEU_AUTO_DOWNLOAD.toString() + " - " + this.getMiuiVersion().toString());
      }
