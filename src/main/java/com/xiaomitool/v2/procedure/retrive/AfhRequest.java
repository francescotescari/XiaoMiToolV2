...
public class AfhRequest {
  ...
  public static String getDownloadLink(String fid) throws CustomHttpException, RomException {
    ...
    EasyResponse response =
        new EasyHttp().url(url).referer(AFH_HOST).userAgent(EasyHttp.CHROME_USERAGENT).exec();
    ...
    response =
        new EasyHttp()
            ...
            .cookies(cookies)
            .fields(fields)
            .userAgent(EasyHttp.CHROME_USERAGENT)
  ...
  public static List<AfhEntry> listDirFiles(String dirId) throws CustomHttpException, RomException {
    ...
    EasyResponse response =
        new EasyHttp().url(url).userAgent(EasyHttp.CHROME_USERAGENT).referer(AFH_HOST).exec();
    if (!response.isAllRight()) {
      ...
      for (int i = 0; i < array.length(); ++i) {
        try {
          ...
          String filename = obj.getString("name"),
              ...
              upload = obj.getString("upload_date");
