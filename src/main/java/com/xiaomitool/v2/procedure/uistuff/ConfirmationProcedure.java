    ...
    return RNode.sequence(
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
            ...
            Procedures.saveProcedure("confirmInstallableProcedure", confirmInstallableProcedure())
                .run(runner);
            Installable installable = Procedures.requireInstallable(runner);
            Device device = Procedures.requireDevice(runner);
            List<InstallationRequirement> requirements =
                InstallationRequirement.getAllInstallableRequirements(installable, device);
            ...
            StringBuilder text =
                new StringBuilder(
                    LRes.CONFIRM_REQUIREMENTS_TEXT.toString(
                        LRes.CONTINUE.toString(), LRes.CANCEL.toString()));
        ...
        RNode.conditional(
            KEY_BOOL_CONFIRM_STEPS,
            null,
            RNode.sequence(
                ChooseProcedure.chooseRom(),
                Procedures.runSavedProcedure("confirmInstallableProcedure"))));
  }
  ...
  public static RInstall confirmInstallationStart() {
    return RNode.sequence(
        new RInstall() {
          ...
          public void run(ProcedureRunner runner)
              throws InstallException, RMessage, InterruptedException {
            Procedures.saveProcedure("confirmInstallationStart", confirmInstallationStart())
                .run(runner);
        ...
        RNode.conditional(
            KEY_BOOL_CONFIRM_INSTALL,
            null,
            RNode.sequence(
                ChooseProcedure.chooseRom(),
                confirmInstallableProcedure(),
                Procedures.runSavedProcedure("confirmInstallationStart"))));
  }
