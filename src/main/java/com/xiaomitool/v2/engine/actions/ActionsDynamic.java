...
public class ActionsDynamic {
  ...
  public static RunnableMessage NO_DEVICE_CONNECTED(Device.Status... wantedStatus) {
    return () -> {
      ...
      Device.Status wStatus =
          (wantedStatus == null || wantedStatus.length == 0) ? null : wantedStatus[0];
      if (wStatus == null) {
      ...
      switch (wStatus) {
        ...
        case RECOVERY:
          msg =
              LRes.NO_DEVICE_CONNECTED_MODE.toString(
                  Device.Status.RECOVERY.toString(), Device.Status.RECOVERY.toString());
        ...
        case FASTBOOT:
          msg =
              LRes.NO_DEVICE_CONNECTED_MODE.toString(
                  Device.Status.FASTBOOT.toString(), Device.Status.FASTBOOT.toString());
        ...
        default:
          ...
          howto =
              ActionsDynamic.HOW_TO_ENABLE_USB_DEBUGGING(
                  null, DeviceManager.getDevices().isEmpty());
      }
      ButtonPane pane = new ButtonPane(LRes.SEARCH_AGAIN, button);
      ImageView no_connection =
          new ImageView(
              new Image(DrawableManager.getPng(DrawableManager.NO_CONNECTION).toString()));
      ...
      while ((message != CommonsMessages.NEW_DEVICE || DeviceManager.count(wantedStatus) == 0)
          && message < 0) {
  ...
  public static RunnableMessage SELECT_DEVICE(@Nullable Device.Status... wantedStatus) {
    return () -> {
      ...
      TableView<DeviceTableEntry> tableView =
          new TableView<DeviceTableEntry>() {
      ...
      for (Device device : DeviceManager.getDevices()) {
        if (wantedStatus != null
            && wantedStatus.length > 0
            && !ArrayUtils.in(wantedStatus, device.getStatus())) {
          continue;
        }
  ...
  public static RunnableMessage REQUIRE_DEVICE_ON(Device device) {
    return () -> {
      Log.info(
          "Rebooting and expecting device "
              + device.getSerial()
              + " turned on with usb debug enabled, auth and connected");
    ...
    return () -> {
      ...
      LRes[] kstring =
          new LRes[] {
            ...
            LRes.CODENAME,
            LRes.MIUI_VERSION,
            LRes.ANDROID_VERSION,
            LRes.SERIAL_NUMBER,
            LRes.BOOTLOADER_STATUS,
            LRes.FASTBOOT_PARSED,
            LRes.RECOVERY_PARSED
          };
              ...
              new Runnable() {
                ...
                public void run() {
                  ...
                  if (!device.getDeviceProperties().getSideloadProperties().isParsed()
                      && !device.getDeviceProperties().getRecoveryProperties().isParsed()) {
                    try {
                      ...
                      if (UnlockStatus.UNLOCKED.equals(device.getAnswers().getUnlockStatus())) {
                        Log.info(
                            "Bootloader is unlocked, we dont care about recovery properties now");
                        Platform.runLater(
                            new Runnable() {
                              @Override
                              public void run() {
                    ...
                    Log.info(
                        "Either stock recovery or custom recovery properties are already found, we dont need more fore now, skipping");
                    ActionsUtil.setDevicePropertiesText(device, texts);
                  }
                  try {
                  ...
                  try {
                    ...
                    ActionsDynamic.START_PROCEDURE(
                            device, GenericInstall.main(), null, GenericInstall.selectRomAndGo())
                        .run();
  ...
  public static RunnableMessage REQUIRE_DEVICE_AUTH(Device device) {
    return () -> {
      ...
      ImageView image =
          new ImageView(DrawableManager.getPng(DrawableManager.DEVICE_AUTH).toString());
  ...
  public static RunnableMessage START_PROCEDURE(
      Device device, RInstall startFromHere, ProcedureRunner runner, RInstall onRestart) {
    return new RunnableMessage() {
      @Override
      public int run() throws InterruptedException {
        ...
        try {
          try {
            ...
            if (startFromHere == null) {
              throw new InstallException(
                  "Null procedure to start from", InstallException.Code.INTERNAL_ERROR);
            }
            ...
            try {
              ...
              thisRunner.handleException(
                  new InstallException(e.getMessage(), InstallException.Code.INTERNAL_ERROR, e),
                  main);
          ...
          PopupWindow popupWindow =
              new PopupWindow.ImageTextPopup(
                  "A fatal error has occured: type: "
                      + rMessage.getCmd()
                      + "\nThe tool will now exit",
                  PopupWindow.Icon.ERROR);
  ...
  public static RunnableMessage HOWTO_GO_FASTBOOT(Device device, boolean canFail) {
    return () -> {
      ButtonPane buttonPane =
          canFail
              ? new ButtonPane(LRes.OK_FINISHED, LRes.FAILED_TO_DO_THAT)
              : new ButtonPane(LRes.OK_FINISHED);
      ...
      while (msg < 0) {
        if (device != null
            && device.isConnected()
            && (Device.Status.FASTBOOT.equals(device.getStatus()))) {
          exitcode = 1;
          break;
        }
        if (device == null) {
  ...
  public static RunnableMessage HOWTO_GO_RECOVERY(boolean rebootingText, Device device) {
    return () -> {
      ...
      while (msg < 0) {
        ...
        if (device != null
            && device.isConnected()
            && (Device.Status.SIDELOAD.equals(device.getStatus())
  ...
  public static RunnableMessage HOW_TO_ENABLE_USB_DEBUGGING(Device device, boolean search) {
    return new RunnableMessage() {
      ...
      public int run() throws InterruptedException {
        String[] text =
            new String[] {
              LRes.HT_ENABLE_USB_1.toString(),
              LRes.HT_ENABLE_USB_2.toString(),
              LRes.HT_ENABLE_USB_3.toString(),
              LRes.HT_ENABLE_USB_4.toString(),
              LRes.HT_ENABLE_USB_5.toString(),
              LRes.HT_ENABLE_USB_6.toString(),
              LRes.HT_ENABLE_USB_7.toString(),
            };
        ...
        DeviceImgInstructionPane instructionPane =
            new DeviceImgInstructionPane(
                WindowManager.getContentHeight() + 30,
                WindowManager.getContentHeight() - 10,
                text,
                images,
                animations);
        ...
        while (message < 0) {
          exitcode = 0;
          if (device != null) {
            if (Device.Status.DEVICE.equals(device.getStatus())
                || Device.Status.UNAUTHORIZED.equals(device.getStatus())) {
              exitcode = 1;
              break;
            }
          } else {
            if (message == CommonsMessages.NEW_DEVICE && !DeviceManager.getDevices().isEmpty()) {
              ...
              if (d != null
                  && (Device.Status.DEVICE.equals(d.getStatus())
                      || Device.Status.UNAUTHORIZED.equals(d.getStatus()))) {
                exitcode = 1;
                break;
              }
            }
  ...
  public static RunnableMessage SELECT_DEVICE_CODENAME(
      Pointer result, HashMap<String, String> devices) {
    return new RunnableMessage() {
      @Override
      public int run() throws InterruptedException {
        Log.info("Displaying codename choose table");
        TableView<DeviceCodenameEntry> tableView =
            new TableView<DeviceCodenameEntry>() {
              public void requestFocus() {}
            };
        TableColumn<DeviceCodenameEntry, String> codename =
            new TableColumn<>(LRes.CODENAME.toString());
        ...
        for (Map.Entry<String, String> device : sortingList) {
          DeviceCodenameEntry tableEntry =
              new DeviceCodenameEntry(device.getKey(), device.getValue());
          observableList.add(tableEntry);
        }
