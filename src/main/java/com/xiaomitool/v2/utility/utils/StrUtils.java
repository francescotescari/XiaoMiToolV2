...
public class StrUtils {
  ...
  private static final Pattern PROGRESS_REGEX =
      Pattern.compile("\\[\\s*(\\d+)\\s*\\/\\s*(\\d+)\\s*\\]");
  private static final byte[] SPACE_STRING =
      "                                                ".getBytes(ResourcesConst.interalCharset());
  private static final byte[] TABS_STIRNG =
      "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t"
          .getBytes(ResourcesConst.interalCharset());
