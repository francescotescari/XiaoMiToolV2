...
public class CustomHttpRequest {
  ...
  private static HttpResponse execRequest(
      HttpClientBuilder client, HttpUriRequest request, boolean useTLS12)
      throws CustomHttpException {
    ...
    if (requestType.equals(Type.POST)) {
      ...
      if (this.postData != null) {
        ...
        for (Map.Entry<String, Object> entry : postParams.entrySet()) {
          basicNameValuePairs.add(
              new BasicNameValuePair(entry.getKey(), entry.getValue().toString()));
        }
        ((HttpPost) request)
            .setEntity(
                new UrlEncodedFormEntity(basicNameValuePairs, ResourcesConst.interalCharset()));
      }
