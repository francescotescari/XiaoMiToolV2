...
public class ProgressPane extends VBox {
    ...
    public static class DefProgressPane extends ProgressPane{
        ...
        public UpdateListener getUpdateListener(int updateEveryMillis){
            ...
            advancedUpdateListener.addOnStart(new UpdateListener.OnStart() {
                @Override
                public void run(long totalSize) {
                    setText(LRes.STARTING_TASK);
                }
            });
            advancedUpdateListener.addOnAdvancedUpdate(new AdvancedUpdateListener.OnAdvancedUpdate() {
                @Override
                public void run(long downloaded, long totalSize, AdvancedUpdateListener.DownloadSpeed currentSpeed, AdvancedUpdateListener.DownloadSpeed averageSpeed, AdvancedUpdateListener.TimeRemaining missingTime) {
                    ...
                    String text;
                    if (totalSize != 100){
                        text = LRes.PROGRESS_TEXT.toString(DECIMAL_FORMAT.format(sp2), DECIMAL_FORMAT.format(sp), UNITS[index]+"B",(int) (progress*100),averageSpeed.toString(),missingTime.getQuantity(),measure.toString());
                    } else {
                        text = LRes.PROGRESS_TEXT.toString(DECIMAL_FORMAT.format(sp2), DECIMAL_FORMAT.format(sp), "%",(int) (progress*100),"",missingTime.getQuantity(),measure.toString());
                    }
                    setText(text);
                }
            });
            advancedUpdateListener.addOnFinished(new UpdateListener.OnFinished() {
                @Override
